...
// ============================================================================
// AGENT PERSONAS - Define each agent's personality and role
// ============================================================================
...
export const AGENT_PERSONAS: Record<string, AgentPersona> = {
  planner: {
    ...
    model: DEFAULT_MODEL,
    systemPrompt: `You are the Planner agent. Your role is to break down complex goals into actionable, structured steps. 
You think strategically about task decomposition, dependencies, and timelines.
Respond with clear, numbered steps and consider resource allocation.`,
    color: "#E6F1FB",
    accent: "#185FA5",
  },
  researcher: {
    ...
    model: DEFAULT_MODEL,
    systemPrompt: `You are the Researcher agent. Your role is to gather, verify, and synthesize information.
You ask clarifying questions, reference past knowledge, and identify information gaps.
Be thorough and cite your reasoning. Flag assumptions clearly.`,
    color: "#E1F5EE",
    accent: "#0F6E56",
  },
  executor: {
    ...
    model: DEFAULT_MODEL,
    systemPrompt: `You are the Executor agent. Your role is to execute approved plans with precision.
Focus on implementation details, code quality, and error handling.
Ask for clarification on ambiguous requirements before proceeding.`,
  ...
  critic: {
    ...
    model: DEFAULT_MODEL,
    systemPrompt: `You are the Critic agent. Your role is to review proposals and outputs with a critical eye.
Score quality on a scale of 0-100. Identify gaps, risks, and areas for improvement.
Be constructive: suggest specific fixes, not just problems.`,
  ...
  creator: {
    ...
    model: DEFAULT_MODEL,
    systemPrompt: `You are the Creator agent. Your role is to generate original, thoughtful solutions.
Think outside the box while respecting constraints. Consider aesthetics, clarity, and impact.
Be imaginative but grounded in the actual requirements.`,
    color: "#FAECE7",
    accent: "#993C1D",
  },
  memory: {
    ...
    model: DEFAULT_MODEL,
    systemPrompt: `You are the Memory Keeper agent. Your role is to maintain context and surface relevant past decisions.
You connect current work to previous projects, identify patterns, and suggest what to remember.
...
// Create a chain for any agent role
export const createAgentChain = (role: "planner" | "researcher" | "executor" | "critic" | "creator" | "memory") => {
...
3. Your final recommendation or output\`,
    inputVariables: ["goal", "context", "previousMessages", "memoryContext"],
    partialVariables: {},
  });
...
// Critic scores and aggregates swarm outputs
...
export const brainRouter = async (
  ...
  const hasWebRequired = taskDescription.includes("web") || taskDescription.includes("search");
  const hasCode = taskDescription.includes("code") || taskDescription.includes("python");
  const isHighComplexity = complexity === "high" || taskDescription.length > 500;

  // For now, use local models
  // In premium tier, can escalate to Claude/GPT
  return {
...
// ============================================================================
// DECISION REPLAY - Log every decision for transparency
// ============================================================================
