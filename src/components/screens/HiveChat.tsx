...
export const HiveChat: React.FC<HiveChatProps> = () => {
  ...
  const handleSendReport = async () => {
    ...
    const reportTitle = `HiveMind Swarm Report: ${currentSession.goal.text}`;
    const reportBody = `
...
Generated by HiveMind Swarm Orchestrator.
    `.trim();

    const { recipientEmail, smtpConfig } = useSwarmStore.getState();
    
    if (!smtpConfig.useDirect || !smtpConfig.pass || smtpConfig.pass === 'your-app-password-here') {
      // Fallback to mailto if direct sending not enabled or configured
      ...
      try {
        ...
        alert('Could not open email client. Report copied to clipboard.');
      }
      return;
    }

    try {
      // Attempt direct background sending via Rust backend
      ...
      alert('🚀 Email sent successfully via HiveMind background agent!');
    } catch (err) {
      console.error('Failed to send direct email:', err);
      const errorMsg = String(err);
      
      // If it's a specific SMTP error, let the user know
      if (errorMsg.includes('Could not send email') || errorMsg.includes('Invalid')) {
        alert(`❌ Background Email Failed: ${errorMsg}\n\nPlease check your SMTP settings or App Password.`);
      } else {
        // Fallback to mailto only if it's not a configuration error
        const mailtoUrl = `mailto:${recipientEmail}?subject=${encodeURIComponent(reportTitle)}&body=${encodeURIComponent(reportBody.substring(0, 1500))}`;
        await openUrl(mailtoUrl);
      }
    }
